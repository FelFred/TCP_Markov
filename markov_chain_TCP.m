...
while 1 
    for cwnd = cwnd_min:cwnd_max
        weight(min(cwnd_max,cwnd+1)) = weight(min(cwnd_max,cwnd+1)) + prev_weight(cwnd)*(1-p)^cwnd;
        weight(max(cwnd_min, ceil(cwnd/2))) = weight(max(cwnd_min, ceil(cwnd/2))) + prev_weight(cwnd)*(1-(1-p)^cwnd);
    end
